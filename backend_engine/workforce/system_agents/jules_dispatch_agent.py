...
class JulesDispatchAgent(BaseAgent):
    ...
    def dispatch_task(
        ...
        from anatomy.api_vault import get_jules_key, report_jules_429
        jules_key = get_jules_key()
        if not jules_key:
            error = "No Jules API Keys available in Vault. Cannot dispatch."
        ...
        # 4. Call Jules API
        self.set_state(AgentState.CODING, f"Dispatching session for {repo_owner}/{repo_name}")
        max_retries = 3
        retries = 0
        ...
        while retries < max_retries:
            ...
            if not result["success"] and "429" in str(result.get("error", "")):
                logger.warning("🔮 JULES: Hit 429 Rate Limit. Reporting to Vault and retrying...")
                report_jules_429(jules_key)
                retries += 1
                continue
        ...
        create_jules_session(
            ...
            repo_source=repo,
            branch=branch,
            api_key_used=jules_key[-4:] if jules_key else "",
        )
    ...
    def check_session(self, session_id: str) -> dict:
        ...
        """
        from anatomy.api_vault import get_jules_key
        jules_key = get_jules_key()
        
        self.set_state(AgentState.INGESTING, f"Checking session: {session_id}")

        result = get_session(jules_key, session_id)
    ...
    def approve_session(self, session_id: str) -> dict:
        ...
        result = approve_plan(jules_key, session_id)
    ...
    def reply_to_session(self, session_id: str, message: str) -> dict:
        """
        Send a follow-up message to a Jules session (used by QA Agent).
        """
        from anatomy.api_vault import get_jules_key
        jules_key = get_jules_key()
        
        self.set_state(AgentState.CODING, f"Replying to session: {session_id}")
        result = send_message(jules_key, session_id, message)
