...
class Orchestrator:
    ...
    async def _poll_cycle(self):
        ...
        for task in pending:
            ...
            if not agent_id:
                logger.warning("🎯 No agent mapped for task_type=%s", task_type)
                continue
    ...
    async def _dispatch_to_agent(self, task_type: str, agent_id: str, description: str, inputs: dict) -> dict:
        ...
        if task_type == "WRITE_ARCH":
            from workforce.tier1_executives.ceo_agent import CEOAgent
            ceo = CEOAgent()
            ...
            from workforce.tier3_local_swarm.qa_agent import QAAgent
            qa = QAAgent()
            ...
            from workforce.tier3_local_swarm.ops_agent import OpsAgent
            ops = OpsAgent()
            pr_number = inputs.get("pr_number", 0)
            repo_path = inputs.get("repo_path", ".")
